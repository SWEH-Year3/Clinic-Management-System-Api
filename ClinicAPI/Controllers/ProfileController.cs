...
namespace ClinicAPI.Controllers
{
    ...
    public class ProfileController : ControllerBase
    {
        ...
        public async Task<IActionResult> setProfilePatient(Guid id, GetProfileResponseDto data )
        {
            // Assuming you have a service to get the profile
            var profile = await profileRepository.UpdateProfileAsync(id, data);
            if (profile == null)
            {
                return NotFound();
            }
        ...
        [Route("doctor/{id:guid}")]
        [RoleAuthorize("Doctor")]
        public async Task<IActionResult> setProfileDoctor(Guid id, DoctorProfileDto data)
        {
            var profile = await profileRepository.UpdateProfileDoctorAsync(id, data);
            if (profile == null)
            {
                return NotFound();
            }
